...
export class StripeBillingProvider implements BillingProvider {
  constructor(private readonly secretKey: string, private readonly prices: Record<BillingPlan, string>) {}
  ...
  async verifyWebhook(payload: string, signature: string, toleranceSeconds = 300): Promise<boolean> {
    ...
    const actual = await hmacSha256(this.secretKey, `${timestamp}.${payload}`);
