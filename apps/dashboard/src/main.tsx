...
type View = "inbox" | "insights" | "moderation" | "roadmap" | "changelog" | "audit" | "developer" | "team" | "notifications" | "billing" | "widget" | "settings" | "privacy";
...
function App() {
  ...
  const activeOrganization = organizations.find((organization) => organization.id === activeProject?.organizationId) ?? organizations[0];
  ...
  const loadView = async (nextView: View) => {
    ...
    try {
      ...
      if (nextView === "team" && activeOrganization) setMembers((await api<{ items: MemberItem[] }>(`/dashboard/organizations/${activeOrganization.id}/members`, credentials)).items);
      ...
      if (nextView === "privacy") setSettings(await api<Settings>(`/dashboard/projects/${projectId}/settings?projectId=${projectId}`, credentials));
      if (nextView === "widget") {
        const [categoryResult, settingsResult] = await Promise.all([
          api<{ items: Category[] }>(`/dashboard/projects/${projectId}/categories`, credentials),
          api<Settings>(`/dashboard/projects/${projectId}/settings?projectId=${projectId}`, credentials),
        ]);
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <nav className="nav" aria-label="Main navigation">
        ...
        <NavItem active={view === "widget"} icon="▣" label="Widget builder" onClick={() => loadView("widget")} />
    ...
    <main className="main-content">
      ...
      <div className="content-wrap">
        ...
        {view === "team" && <Team members={members} organizationId={activeOrganization?.id ?? ""} credentials={credentials} onRefresh={() => void loadView("team")} />}
        ...
        {view === "widget" && <WidgetBuilder settings={settings} categories={categories} projectId={projectId} publicKey={publicKey} credentials={credentials} onSettings={setSettings} onCategories={setCategories} onNotice={setNotice} />}
        {view === "roadmap" && <Roadmap items={roadmap} credentials={credentials} projectId={projectId} feedback={feedback} onChange={() => void loadView("roadmap")} />}
        {view === "changelog" && <Changelog items={changelog} credentials={credentials} projectId={projectId} feedback={feedback} onChange={() => void loadView("changelog")} />}
        ...
        {view === "privacy" && <PrivacyPanel feedback={feedback} settings={settings} projectId={projectId} credentials={credentials} onRefresh={loadInbox} />}
      </div>
    </main>
  </div>;
}
...
function FeedbackLinks({ kind, itemId, projectId, credentials, feedback }: { kind: "roadmap" | "changelog"; itemId: string; projectId: string; credentials: ApiOptions; feedback: Feedback[] }) {
  ...
  const refresh = async () => { const result = await api<{ items: FeedbackLink[] }>(endpoint, credentials); setLinked(result.items); };
  useEffect(() => { void refresh(); }, [endpoint]);
  const link = async () => { if (!selected) return; await api(endpoint, { ...credentials, method: "POST", body: JSON.stringify({ feedbackId: selected }) }); setSelected(""); await refresh(); };
  const unlink = async (feedbackId: string) => { await api(`${endpoint}/${encodeURIComponent(feedbackId)}`, { ...credentials, method: "DELETE" }); await refresh(); };
  const available = feedback.filter((item) => !linked.some((entry) => entry.id === item.id));
  return <div className="feedback-links"><div className="feedback-links-heading"><strong>Linked feedback</strong><span>{linked.length}</span></div><div className="feedback-link-form"><select value={selected} onChange={(event) => setSelected(event.target.value)} aria-label={`Link feedback to ${kind}`}><option value="">Select feedback…</option>{available.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select><button className="secondary-button" disabled={!selected} onClick={() => void link()}>Link</button></div>{linked.length > 0 && <div className="linked-feedback-list">{linked.map((item) => <div className="linked-feedback" key={item.id}><span className={`type-dot ${item.type}`} /><span title={item.title}>{item.title}</span><button className="text-danger" onClick={() => void unlink(item.id)} aria-label={`Unlink ${item.title}`}>×</button></div>)}</div>}</div>;
}

function Roadmap({ items, credentials, projectId, feedback, onChange }: { items: Array<Record<string, string>>; credentials: ApiOptions; projectId: string; feedback: Feedback[]; onChange: () => void }) { const [title, setTitle] = useState(""); const [body, setBody] = useState(""); const add = async () => { if (!title.trim()) return; await api(`/dashboard/projects/${projectId}/roadmap?projectId=${projectId}`, { ...credentials, method: "POST", body: JSON.stringify({ title, body, status: "planned" }) }); setTitle(""); setBody(""); onChange(); }; return <><PageHeader eyebrow="Product direction" title="Roadmap" description="Turn recurring feedback into visible product progress." action={<button className="primary-button" onClick={add}>＋ <span>Add item</span></button>} /><section className="panel composer"><input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Add a roadmap item title…" /><textarea value={body} onChange={(event) => setBody(event.target.value)} placeholder="What are you planning to build?" /><button className="primary-button" onClick={add}>Create roadmap item</button></section><div className="roadmap-grid">{["planned", "in_progress", "completed"].map((status) => <section className="roadmap-column" key={status}><div className="column-heading"><span className={`column-dot ${status}`} /><h2>{statusLabel(status)}</h2><span>{items.filter((item) => item.status === status).length}</span></div>{items.filter((item) => item.status === status).map((item) => <article className="roadmap-card" key={item.id}><span className="card-kicker">Product</span><h3>{item.title}</h3><p>{item.body}</p><FeedbackLinks kind="roadmap" itemId={item.id} projectId={projectId} credentials={credentials} feedback={feedback} /><small>Updated {item.updatedAt ? formatDate(item.updatedAt) : "recently"}</small></article>)}</section>)}</div></> }

function Changelog({ items, credentials, projectId, feedback, onChange }: { items: Array<Record<string, string | null>>; credentials: ApiOptions; projectId: string; feedback: Feedback[]; onChange: () => void }) { const [title, setTitle] = useState(""); const [body, setBody] = useState(""); const add = async () => { if (!title.trim() || !body.trim()) return; await api(`/dashboard/projects/${projectId}/changelog?projectId=${projectId}`, { ...credentials, method: "POST", body: JSON.stringify({ title, body }) }); setTitle(""); setBody(""); onChange(); }; return <><PageHeader eyebrow="Keep users in the loop" title="Changelog" description="Share shipped work and close the feedback loop with your users." action={<button className="primary-button" onClick={add}>＋ <span>New update</span></button>} /><section className="panel composer"><input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Update title…" /><textarea value={body} onChange={(event) => setBody(event.target.value)} placeholder="What changed?" /><button className="primary-button" onClick={add}>Publish update</button></section><section className="timeline">{items.length === 0 ? <div className="empty-state"><div className="empty-icon">✦</div><strong>No updates published yet</strong><p>Your changelog will appear here.</p></div> : items.map((item) => <article className="timeline-item" key={item.id}><div className="timeline-marker" /><div className="panel"><div className="timeline-meta"><span className="status-pill resolved">Published</span><time>{item.publishedAt ? formatDate(item.publishedAt) : "Draft"}</time></div><h2>{item.title}</h2><p>{item.body}</p><FeedbackLinks kind="changelog" itemId={item.id ?? ""} projectId={projectId} credentials={credentials} feedback={feedback} /></div></article>)}</section></> }

function WidgetBuilder({ settings, categories, projectId, publicKey, credentials, onSettings, onCategories, onNotice }: { settings: Settings | null; categories: Category[]; projectId: string; publicKey: string; credentials: ApiOptions; onSettings: (value: Settings) => void; onCategories: (value: Category[]) => void; onNotice: (value: { text: string; error?: boolean }) => void }) {
  ...
  const value = (key: string, fallback: string) => typeof theme[key] === "string" ? String(theme[key]) : fallback;
  const mode = value("mode", "floating");
  const fields = Array.isArray(theme.fields) ? theme.fields.filter((field): field is string => typeof field === "string") : ["type", "title", "description", "email"];
  const updateTheme = (key: string, next: unknown) => setDraft(draft ? { ...draft, theme: { ...draft.theme, [key]: next } } : draft);
  const save = async () => {
    if (!draft) return;
    setSaving(true);
    try {
      const updated = await api<Settings>(`/dashboard/projects/${projectId}/settings?projectId=${projectId}`, { ...credentials, method: "PATCH", body: JSON.stringify(draft) });
      onSettings(updated); onNotice({ text: "Widget configuration saved" });
    } catch (error) { onNotice({ text: error instanceof Error ? error.message : "Unable to save widget configuration", error: true }); }
    finally { setSaving(false); }
  };
  const addCategory = async () => {
    if (categoryName.trim().length < 2) return;
    try {
      const created = await api<Category>(`/dashboard/projects/${projectId}/categories`, { ...credentials, method: "POST", body: JSON.stringify({ name: categoryName.trim() }) });
      onCategories([...categories, created].sort((left, right) => left.name.localeCompare(right.name))); setCategoryName(""); onNotice({ text: "Category added" });
    } catch (error) { onNotice({ text: error instanceof Error ? error.message : "Unable to add category", error: true }); }
  };
  const removeCategory = async (category: Category) => {
    if (!window.confirm(`Delete the ${category.name} category? Existing feedback will keep its other data.`)) return;
    try {
      await api(`/dashboard/projects/${projectId}/categories/${encodeURIComponent(category.id)}`, { ...credentials, method: "DELETE" });
      onCategories(categories.filter((item) => item.id !== category.id)); onNotice({ text: "Category deleted" });
    } catch (error) { onNotice({ text: error instanceof Error ? error.message : "Unable to delete category", error: true }); }
  };
  ...
  const copy = async () => { await navigator.clipboard?.writeText(snippet); setCopied(true); setTimeout(() => setCopied(false), 1800); };
  return <><PageHeader eyebrow="Activation" title="Widget builder" description="Shape the feedback experience, preview it, and copy the exact Web SDK setup." action={<button className="primary-button" onClick={() => void save()} disabled={saving}>{saving ? "Saving…" : "Save widget"}</button>} /><div className="widget-builder-grid"><section className="panel settings-card"><div className="panel-heading"><div><h2>Appearance & behavior</h2><p>These settings are stored with the project and can be mirrored in any SDK.</p></div></div><label className="form-field"><span>Display mode</span><select value={mode} onChange={(event) => updateTheme("mode", event.target.value)}><option value="floating">Floating button</option><option value="modal">Modal</option><option value="side-panel">Side panel</option><option value="inline">Inline form</option><option value="portal">Full-page portal</option><option value="headless">Headless</option></select></label><div className="color-fields"><label className="form-field"><span>Primary color</span><input type="color" value={value("primary", "#7C3AED")} onChange={(event) => updateTheme("primary", event.target.value)} /></label><label className="form-field"><span>Background</span><input type="color" value={value("background", "#FFFFFF")} onChange={(event) => updateTheme("background", event.target.value)} /></label><label className="form-field"><span>Text color</span><input type="color" value={value("text", "#181221")} onChange={(event) => updateTheme("text", event.target.value)} /></label></div><label className="form-field"><span>Button label</span><input value={value("buttonLabel", "Give feedback")} onChange={(event) => updateTheme("buttonLabel", event.target.value)} maxLength={40} /></label><div className="field-picker"><span>Form fields</span>{["type", "category", "title", "description", "attachment", "email"].map((field) => <label key={field}><input type="checkbox" checked={fields.includes(field)} onChange={(event) => updateTheme("fields", event.target.checked ? [...fields, field] : fields.filter((entry) => entry !== field))} />{statusLabel(field)}</label>)}</div></section><section className="panel widget-preview-card"><div className="panel-heading"><div><h2>Live preview</h2><p>A lightweight preview of the configured surface.</p></div><span className="period-pill">{mode}</span></div><div className="widget-preview" style={{ background: value("background", "#FFFFFF"), color: value("text", "#181221") }}><div className="preview-top"><span className="preview-mark" style={{ background: value("primary", "#7C3AED") }}>N</span><div><strong>Your feedback</strong><small>Help us make this product better.</small></div></div><div className="preview-input">Tell us what happened…</div><div className="preview-input">Describe your feedback…</div><button style={{ background: value("primary", "#7C3AED") }}>{value("buttonLabel", "Give feedback")}</button></div></section></div><div className="widget-lower-grid"><section className="panel settings-card"><div className="panel-heading"><div><h2>Public categories</h2><p>Categories can be passed as category IDs by the Web, iOS, and Android clients.</p></div></div><div className="developer-form"><input value={categoryName} onChange={(event) => setCategoryName(event.target.value)} placeholder="e.g. Payments" onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); void addCategory(); } }} /><button className="primary-button" onClick={() => void addCategory()} disabled={categoryName.trim().length < 2}>Add category</button></div><div className="category-list">{categories.length === 0 ? <p className="muted">No custom categories yet. The built-in feedback types remain available.</p> : categories.map((category) => <div className="category-row" key={category.id}><span className="category-dot" /><div><strong>{category.name}</strong><small>{category.slug}</small></div><button className="text-danger" type="button" onClick={() => void removeCategory(category)} aria-label={`Delete ${category.name}`}>Delete</button></div>)}</div></section><section className="panel settings-card snippet-card"><div className="panel-heading"><div><h2>Install snippet</h2><p>Give this to your developer or use it as a starting point.</p></div><button className="secondary-button" onClick={() => void copy()}>{copied ? "Copied" : "Copy"}</button></div><pre><code>{snippet}</code></pre></section></div></>;
}

function PrivacyPanel({ feedback, settings, projectId, credentials, onRefresh }: { feedback: Feedback[]; settings: Settings | null; projectId: string; credentials: ApiOptions; onRefresh: () => Promise<void> }) {
  ...
  const exportData = async () => {
    ...
    try {
      ...
      if (credentials.projectKey) headers.set("x-nitroping-project-key", credentials.projectKey);
      if (credentials.serverKey) headers.set("x-nitroping-server-key", credentials.serverKey);
      const response = await fetch(`/api/v1/dashboard/projects/${encodeURIComponent(projectId)}/export`, { credentials: "include", headers });
      if (!response.ok) throw new Error("The data export could not be created.");
      const url = URL.createObjectURL(await response.blob()); const anchor = document.createElement("a"); anchor.href = url; anchor.download = `nitroping-${projectId}-export.json`; anchor.click(); URL.revokeObjectURL(url); setNotice("Your export download has started.");
    } catch (error) { setNotice(error instanceof Error ? error.message : "The data export failed."); }
    finally { setBusy(false); }
  };
  const anonymize = async (item: Feedback) => {
    if (!window.confirm(`Anonymize “${item.title}”? This removes the author email, body, metadata, comments, and attachments.`)) return;
    setBusy(true); setNotice("");
    try { await api(`/dashboard/projects/${encodeURIComponent(projectId)}/feedback/${encodeURIComponent(item.id)}/anonymize`, { ...credentials, method: "POST" }); setNotice("Feedback anonymized."); await onRefresh(); }
    catch (error) { setNotice(error instanceof Error ? error.message : "The feedback could not be anonymized."); }
    finally { setBusy(false); }
  };
  return <><PageHeader eyebrow="Privacy & compliance" title="Privacy & data" description="Export project data, review retention, and remove personal feedback data safely." action={<button className="primary-button" disabled={busy} onClick={() => void exportData()}>{busy ? "Working…" : "Download project export"}</button>} />{notice && <div className="privacy-notice">{notice}</div>}<section className="settings-grid"><div className="panel settings-card"><div className="panel-heading"><div><h2>Retention policy</h2><p>Automatic cleanup follows the project policy.</p></div></div><div className="usage-number"><strong>{settings?.retentionDays ?? 365}</strong><span>days</span></div><p className="muted">Feedback older than this period is anonymized and attachments are queued for physical deletion.</p></div><div className="panel settings-card"><div className="panel-heading"><div><h2>Data export</h2><p>Download feedback, comments, and attachment metadata in JSON.</p></div></div><button className="secondary-button full-button" disabled={busy} onClick={() => void exportData()}>Download export</button></div></section><section className="panel settings-card privacy-feedback"><div className="panel-heading"><div><h2>Feedback anonymization</h2><p>Use this for a verified deletion request or data minimization workflow.</p></div></div>{feedback.length === 0 ? <div className="empty-state"><strong>No feedback to anonymize</strong></div> : feedback.slice(0, 50).map((item) => <div className="privacy-row" key={item.id}><div><strong>{item.title}</strong><small>{item.email ?? "Anonymous"} · {formatDate(item.createdAt)}</small></div><button className="text-danger" disabled={busy || item.body === "[anonymized]"} onClick={() => void anonymize(item)}>{item.body === "[anonymized]" ? "Anonymized" : "Anonymize"}</button></div>)}</section></>;
}

function SettingsPanel({ settings, projectId, publicKey, serverKey, onProject, onPublic, onServer, onSave, onSettings }: { settings: Settings | null; projectId: string; publicKey: string; serverKey: string; onProject: (value: string) => void; onPublic: (value: string) => void; onServer: (value: string) => void; onSave: () => void; onSettings: (value: Settings) => void }) { const [draft, setDraft] = useState(settings); useEffect(() => setDraft(settings), [settings]); const save = async () => { if (!draft) return; const updated = await api<Settings>(`/dashboard/projects/${projectId}/settings?projectId=${projectId}`, { projectKey: publicKey, serverKey, method: "PATCH", body: JSON.stringify(draft) }); onSettings(updated); onSave(); }; return <><PageHeader eyebrow="Control room" title="Project settings" description="Configure how NitroPing fits into your product and team." action={<button className="primary-button" onClick={save}>Save changes</button>} /><section className="settings-grid"><div className="panel settings-card"><div className="panel-heading"><div><h2>Connection</h2><p>Keys used by the dashboard and SDKs.</p></div></div><label className="form-field"><span>Project ID</span><input value={projectId} readOnly aria-readonly="true" /></label><label className="form-field"><span>Public project key</span><input value={publicKey} onChange={(event) => onPublic(event.target.value)} /></label><label className="form-field"><span>Server key <em>development only</em></span><input type="password" value={serverKey} onChange={(event) => onServer(event.target.value)} placeholder="Required for dashboard API calls" /></label><button className="secondary-button full-button" onClick={onSave}>Apply connection</button></div><div className="panel settings-card"><div className="panel-heading"><div><h2>Privacy & data</h2><p>Keep control of retention and custom context.</p></div></div><label className="form-field"><span>Retention period <em>days</em></span><input type="number" min="1" max="3650" value={draft?.retentionDays ?? 365} onChange={(event) => setDraft(draft ? { ...draft, retentionDays: Number(event.target.value) } : draft)} /></label><label className="form-field"><span>Allowed metadata fields <em>comma separated</em></span><input value={draft?.allowedMetadata.join(", ") ?? ""} onChange={(event) => setDraft(draft ? { ...draft, allowedMetadata: event.target.value.split(", ").map((part) => part.trim()).filter(Boolean) } : draft)} placeholder="appVersion, account.plan" /></label><label className="form-field"><span>Allowed widget origins</span><input value={draft?.origins.join(", ") ?? ""} onChange={(event) => setDraft(draft ? { ...draft, origins: event.target.value.split(", ").map((part) => part.trim()).filter(Boolean) } : draft)} placeholder="https://app.example.com" /></label></div></section></> }
