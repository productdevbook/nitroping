...
type Config = { theme?: { buttonLabel?: string; colors?: { primary?: string } }; categories?: Array<{ id: string; name: string; slug: string }> };
const api = "/api/v1";
...
function App() {
  ...
  return <main className="portal-shell" style={{ "--portal-primary": config.theme?.colors?.primary ?? "#7657e8" } as CSSProperties}>
    ...
    {showForm && <FeedbackForm projectId={projectId} onClose={() => setShowForm(false)} onCreated={() => { setShowForm(false); setNotice("Thanks — your feedback is now on the board."); void refresh(); }} />}
    {notice && <button className="portal-notice" onClick={() => setNotice("")}>{notice} ×</button>}
  </main>;
}

function FeedbackForm({ projectId: project, onClose, onCreated }: { projectId: string; onClose: () => void; onCreated: () => void }) {
  const [type, setType] = useState<FeedbackType>("suggestion"); const [title, setTitle] = useState(""); const [body, setBody] = useState(""); const [email, setEmail] = useState(""); const [busy, setBusy] = useState(false); const [error, setError] = useState("");
  const submit = async (event: FormEvent) => { event.preventDefault(); setBusy(true); try { await request(`/projects/${encodeURIComponent(project)}/feedback`, { method: "POST", body: JSON.stringify({ type, title, body, email: email || undefined, platform: "web" }), headers: { "content-type": "application/json", "idempotency-key": crypto.randomUUID() } }); onCreated(); } catch (cause) { setError(cause instanceof Error ? cause.message : "Could not send feedback."); } finally { setBusy(false); } };
  return <div className="portal-modal"><form className="portal-dialog feedback-form" onSubmit={submit}><button type="button" className="dialog-close" onClick={onClose}>×</button><p className="portal-eyebrow">Your voice matters</p><h2>Share feedback</h2><label>What kind?<select value={type} onChange={(event) => setType(event.target.value as FeedbackType)}><option value="suggestion">Suggestion</option><option value="bug">Bug report</option><option value="feature_request">Feature request</option><option value="complaint">Complaint</option></select></label><label>Title<input required minLength={3} maxLength={160} value={title} onChange={(event) => setTitle(event.target.value)} /></label><label>Details<textarea required minLength={3} maxLength={20000} value={body} onChange={(event) => setBody(event.target.value)} /></label><label>Email <small>optional, for updates</small><input type="email" value={email} onChange={(event) => setEmail(event.target.value)} /></label>{error && <p className="form-error">{error}</p>}<button className="portal-submit" disabled={busy}>{busy ? "Sending…" : "Send feedback"}</button></form></div>;
}
