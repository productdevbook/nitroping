...
class WebPushProvider {
  ...
  private async encryptPayload(payload: string, subscription: WebPushSubscription): Promise<{
    ...
    const contentEncryptionKey = this.hkdfExpand(prk, Buffer.from('Content-Encoding: aes128gcm\x00', 'utf8'), 16)
    const nonce = this.hkdfExpand(prk, Buffer.from('Content-Encoding: nonce\x00', 'utf8'), 12)
  ...
  async sendMessage(message: WebPushMessage): Promise<WebPushResponse> {
    try {
      ...
      const { ciphertext, salt, localPublicKey } = await this.encryptPayload(payload, message.subscription)
